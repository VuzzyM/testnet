module initia_std::nft {
    ...
    public fun token_ids<Extension: store + drop + copy>(
        ...
        while (table::prepare<String, Nft<Extension>>(&mut nfts_iter) && vector::length(&res) < (limit as u64)) {
            let (token_id, _token) = table::next<String, Nft<Extension>>(&mut nfts_iter);
    ...
    /// publish nft store
    public entry fun register<Extension: store + drop + copy>(account: &signer) acquires EscrowStore, ModuleStore, NftStore {
        ...
        let nft_store = NftStore<Extension> {
            nfts: table::new<String, Nft<Extension>>(),
    ...
    fun gen_escrow_iter<Extension: store + drop + copy>(addr: address): table::TableIter<EscrowKey, Nft<Extension>> acquires EscrowStore {
        let escrow_store = borrow_global_mut<EscrowStore<Extension>>(nft_address<Extension>());
    ...
    fun fail_register(source: signer) acquires EscrowStore, ModuleStore, NftStore {
        init_module_for_test(&source);
        let cap = make_collection_for_test(&source);
