module initia_std::op_bridge {
    ...
    public entry fun register_token<L2ID, CoinType>(account: &signer) acquires BridgeStore {
        ...
        assert!(l2_addr == account_addr, error::permission_denied(EL2_ADDRESS_MISMATCH));
    ...
    public entry fun deposit_token<L2ID, CoinType>(account: &signer, to: address, amount: u64) acquires BridgeStore, DepositStore {
        initiate_token_bridge<L2ID, CoinType>(account, to, coin::withdraw<CoinType>(account, amount), b"")
    }

    /// user facing l2 deposit function with data input
    public entry fun deposit_token_with_data<L2ID, CoinType>(account: &signer, to: address, amount: u64, data: vector<u8>) acquires BridgeStore, DepositStore {
        initiate_token_bridge<L2ID, CoinType>(account, to, coin::withdraw<CoinType>(account, amount), data)
    }
    ...
    /// 
    public fun initiate_token_bridge<L2ID, CoinType>(
        from: &signer, 
        to: address, 
    ...
    public entry fun finalize_token_bridge<L2ID, CoinType>(
        ...
        // withdrawal proofs and withdraw tx data
        {
            ...
            let withdrawal_hash = {
                ...
                vector::append(&mut withdraw_tx_data, *string::bytes(&type_info::type_name<L2ID>()));
    ...
    fun verify_merkle_proof(
        ...
        initiate_token_bridge<GameID, Token>(from, signer::address_of(to), test_coin, b"");
        ...
        vector::append(&mut output_root_seed, x"22b2ae5ea5aff81e453a645b9f89a79d80020190da77998b48b385c19f11b1e9");
        vector::append(&mut output_root_seed, x"9999999999999999999999999999999999999999999999999999999999999999");
        let output_root = sha3_256(output_root_seed);
        assert!(output_root == x"8e9e0940c912c725e75579443c58c54126e48290126a7a7f5367ee2b2b1eaceb", 1);
        ...
        finalize_token_bridge<GameID, Token>(
            ...
            x"22b2ae5ea5aff81e453a645b9f89a79d80020190da77998b48b385c19f11b1e9",
