...
module initia_std::op_output {
    ...
    struct OutputStore<phantom L2ID> has key {
        outputs: Table<vector<u8>, OutputProposal<L2ID>>,
    ...
    public fun next_block_num<L2ID>(): u64 acquires ConfigStore, OutputStore {
        ...
        let next_block_num = if (next_output_index == 0) {
            ...
            assert!(table::prepare<vector<u8>, OutputProposal<L2ID>>(&mut iter), error::aborted(EUNKNOWN));
            let (_, output_proposal) = table::next<vector<u8>, OutputProposal<L2ID>>(&mut iter);
    ...
    fun test_next_block_num(chain: &signer, proposer: &signer, challenger: address) acquires ConfigStore, OutputStore {
        ...
        while (i < 300) {
            ...
            i = i + 1;
            assert!(next_block_num<TestL2ID>() == 301 + i * 100, 1);
        }
    }
}