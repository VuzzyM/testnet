module initia_std::sft {
    ...
    public fun token_ids<Extension: store + drop + copy>(
        ...
        while (table::prepare<String, Sft<Extension>>(&mut sfts_iter) && vector::length(&res) < (limit as u64)) {
            let (token_id, _token) = table::next<String, Sft<Extension>>(&mut sfts_iter);
    ...
    /// publish sft store
    public entry fun register<Extension: store + drop + copy>(account: &signer) acquires EscrowStore, ModuleStore, SftStore {
        ...
        let sft_store = SftStore<Extension> {
            sfts: table::new<String, Sft<Extension>>(),
    ...
    fun gen_escrow_iter<Extension: store + drop + copy>(addr: address): table::TableIter<EscrowKey, Sft<Extension>> acquires EscrowStore {
        let escrow_store = borrow_global_mut<EscrowStore<Extension>>(sft_address<Extension>());
    ...
    fun fail_register(source: signer) acquires EscrowStore, ModuleStore, SftStore {
        init_module_for_test(&source);
        let cap = make_collection_for_test(&source);
