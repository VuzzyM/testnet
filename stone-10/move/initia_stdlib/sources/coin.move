...
module initia_std::coin {
    ...
    fun deposit_escrow<CoinType>(account_addr: address, coin: Coin<CoinType>) acquires EscrowStore, ModuleStore {
        ...
        *amount = *amount + coin.value;
        ...
        // update escrow table of module store
        {
    ...
    public entry fun register<CoinType>(account: &signer) acquires CoinInfo, CoinStore, EscrowStore, ModuleStore {
        let account_addr = signer::address_of(account);
        assert!(
        ...
        let escrow_balance = escrow_balance<CoinType>(account_addr);
        if (escrow_balance != 0) {
            let coin = withdraw_escrow<CoinType>(account);
    ...
    fun initialize_and_register_fake_money(
        account: &signer,
        decimals: u8,
    ): (BurnCapability<FakeMoney>, FreezeCapability<FakeMoney>, MintCapability<FakeMoney>) acquires CoinInfo, CoinStore, EscrowStore, ModuleStore {
