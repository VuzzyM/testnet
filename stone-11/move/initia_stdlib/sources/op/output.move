...
module initia_std::op_output {
    ...
    public fun create_bridge_address(creator: address, l2_id: &String): address {
        object::create_object_address(creator, generate_brige_seed(l2_id))
    }

    fun generate_brige_seed(l2_id: &String): vector<u8> {
    ...
    fun test_initialize(chain: &signer, proposer: address, challenger: address) acquires ConfigStore {
        ...
        let constructor_ref = object::create_named_object(chain, generate_brige_seed(&l2_id));
    ...
    fun initialize_for_test(
        ...
        let constructor_ref = object::create_named_object(creator, generate_brige_seed(&l2_id));
        let bridge_signer = object::generate_signer(&constructor_ref);
